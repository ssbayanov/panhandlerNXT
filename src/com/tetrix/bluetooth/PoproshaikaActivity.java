...
public class PoproshaikaActivity extends Activity {
	...
	public static final String DEVICE_NAME = "device_name"; // Имя устройства
	public static final String TOAST = "toast"; // Перемееная для вывода
												// сообщений на экран
	...
	private void setupBluetooth() {
		...
		// если адаптер null, значит Bluetooth не поддерживается
		if (mBluetoothAdapter == null) {
			Toast.makeText(this, "Bluetooth не доступен", Toast.LENGTH_LONG)
					.show();
			finish();
			return;
		}
		...
		if (!mBluetoothAdapter.isEnabled()) {
			...
			if (time == 30) {
				Toast.makeText(this, "Bluetooth включить не удалось",
						Toast.LENGTH_LONG).show();
				finish();
				return;
			} else {
				Toast.makeText(this, "Bluetooth включен", Toast.LENGTH_LONG)
						.show();
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case REQUEST_ENABLE_BT:
			...
			if (resultCode == Activity.RESULT_OK) {
				...
				Toast.makeText(this, "Bluetooth не активирован",
						Toast.LENGTH_SHORT).show();
				finish();
			}
		}
